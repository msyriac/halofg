...
for mindex,massbin in enumerate(massbins):
    ...
    r = pickle.load(open(out_dir+"recon_stats_noiseless_"+prefix+".pkl",'r'))
    ...
    pl.hline()
    pl.done(io.dout_dir+"postprocKsz"+prefix+".png")
    ...
    def fit(xfg,yfg,gcut,maptype="148_ksz"):
        prefix = massbin+"_"+maptype+"_x_"+str(xfg)+"_y_"+str(yfg)+"_g_"+str(gcut)
        r = pickle.load(open(out_dir+"recon_stats_noiseless_"+prefix+".pkl",'r'))
        ...
        pl.done(io.dout_dir+"lnlikesKsz"+prefix+".png")
    ...
    try:
        if repeat: raise
        refmean,refsigma,refsn,refbias,refbiasp = np.loadtxt("ff"+massbin+mtype+str(gcut)+".txt",unpack=True)
    except:
        refmean,refsigma,refsn,refbias,refbiasp = fit(False,False,gcut=2000,maptype="148_tsz")
        io.save_cols("ff"+massbin+mtype+str(gcut)+".txt",(refmean,refsigma,refsn,refbias,refbiasp))
    
    # True / True

    try:
        if repeat: raise
        meanXY,sigmaXY,snXY,biasXY,biaspXY = np.loadtxt("tt"+massbin+mtype+str(gcut)+".txt",unpack=True)
    except:
        meanXY,sigmaXY,snXY,biasXY,biaspXY = fit(True,True,gcut=2000,maptype="148_ksz")
        io.save_cols("tt"+massbin+mtype+str(gcut)+".txt",(meanXY,sigmaXY,snXY,biasXY,biaspXY))
    ...
    try:
        if repeat: raise
        meanY,sigmaY,snY,biasY,biaspY = np.loadtxt("ft"+massbin+mtype+str(gcut)+".txt",unpack=True)
    except:
        meanY,sigmaY,snY,biasY,biaspY = fit(True,True,gcut=1000,maptype="148_ksz")
        io.save_cols("ft"+massbin+mtype+str(gcut)+".txt",(meanY,sigmaY,snY,biasY,biaspY))
...
sys.exit()
pl = io.Plotter(ylabel="$\\Delta M / M$")
massbins = ['groups','low','med','high']
#massbins = ['C1','C2','C3','C4']
#massbins = ['$3\\times 10^{13}$','$7\\times 10^{13}$','$2\\times 10^{14}$','$4\\times 10^{14}$']
x = range(len(massbins))
pl.add_err(x,xymeans,yerr=xysigmas,ls="none",marker="o",mew=3,elinewidth=3,markersize=8,label="kSZ contaminated",color="crimson")
pl.add_err(np.array(x)+0.05,ymeans,yerr=ysigmas,ls="none",marker="d",mew=3,markersize=8,elinewidth=3,label="kSZ contaminated + aggresive filtering",color="darkorchid")
plt.xticks(x, massbins, rotation='horizontal')
pl.hline()
pl.legend(loc='lower left',labsize=12)
pl.done(io.dout_dir+"Fig2"+mtype+".pdf")
